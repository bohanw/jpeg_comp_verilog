...
while i<=length(array_y)
    ...
    zz_y=[zz_y zeros(1,array_y(i)) array_y(i+2)];
    i=i+3;
...
while i<=length(array_cb)
    ...
    if array_cb(i)==0 && array_cb(i+1)==0
        zz_cb=[zz_cb zeros(1,64-rem(length(zz_cb),64))];
        EOB=1;
        i=i+2;
        continue;
    end
    zz_cb=[zz_cb zeros(1,array_cb(i)) array_cb(i+2)];
    i=i+3;
end
...
while i<=length(array_cr)
    ...
    if array_cr(i)==0 && array_cr(i+1)==0
        zz_cr=[zz_cr zeros(1,64-rem(length(zz_cr),64))];
        EOB=1;
        i=i+2;
        continue;
    end
    zz_cr=[zz_cr zeros(1,array_cr(i)) array_cr(i+2)];
...
for i=1:x_max/8
    for j=1:y_max/8
        ...
        check=rgb_matrix(x_cord:x_cord+7,y_cord:y_cord+7,:);
    end
end
